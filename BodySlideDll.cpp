...
wxIMPLEMENT_APP_NO_MAIN(BodySlideApp);
...
#ifdef _DEBUG
		const HINSTANCE
			hInstance = wxDynamicLibrary::MSWGetModuleHandle("BodySlide x64 Debug",
				&gs_wxMainThread);
#else
		const HINSTANCE
			hInstance = wxDynamicLibrary::MSWGetModuleHandle("BodySlide x64 Release",
				&gs_wxMainThread);
#endif // _DEBUG
...
extern "C"
{
	...
	void launch_bodyslide() {
		bodyslide_closed();
		run_wx_gui_from_dll("BodySlideApp");
	}
	...
	void set_body_slide_event_handler(callback_action act)
	{
		BSDllController::set_event_handler(act);
	}

	bool is_bodyslide_running() {
